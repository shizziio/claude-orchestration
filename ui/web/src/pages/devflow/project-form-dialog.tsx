...
import type { CreateProjectInput, GitCredential } from "@/types/devflow";
...
export function ProjectFormDialog({ open, onClose, onSubmit, credentials }: Props) {
  ...
  const set = (k: keyof CreateProjectInput, v: string) =>
    setForm((f) => ({ ...f, [k]: v }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setForm({ name: "", slug: "", description: "", repo_url: "", default_branch: "main", git_credential_id: "" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-md max-sm:inset-0 max-sm:translate-x-0 max-sm:translate-y-0 max-sm:rounded-none">
        <DialogHeader>
          <DialogTitle>{t("form.newProject")}</DialogTitle>
        </DialogHeader>
